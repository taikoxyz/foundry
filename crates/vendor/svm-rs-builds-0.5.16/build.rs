...
const EMBEDDED_MACOS_AARCH64_RELEASES: &str = include_str!("macosx-aarch64.json");
...
fn generate() {
    ...
    } else if let Some(json) = embedded_release_list(platform) {
        serde_json::from_str(json).expect("Failed to parse embedded releases JSON")
    } else {
...
fn embedded_release_list(platform: svm::Platform) -> Option<&'static str> {
    match platform {
        svm::Platform::MacOsAarch64 => Some(EMBEDDED_MACOS_AARCH64_RELEASES),
        _ => None,
    }
}

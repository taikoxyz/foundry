    ...
    if let Ok(chain) = NamedChain::try_from(env.cfg.chain_id) {
        ...
        match chain {
            ...
            BinanceSmartChain | BinanceSmartChainTestnet => {
                ...
                // failure.
                env.blocks.get_mut(&env.cfg.chain_id).unwrap().prevrandao = Some(env.blocks.get(&env.cfg.chain_id).unwrap().difficulty.into());
                return;
            }
            Moonbeam | Moonbase | Moonriver | MoonbeamDev | Rsk | RskTestnet => {
                if env.blocks.get_mut(&env.cfg.chain_id).unwrap().prevrandao.is_none() {
                    // <https://github.com/foundry-rs/foundry/issues/4232>
                    env.blocks.get_mut(&env.cfg.chain_id).unwrap().prevrandao = Some(B256::random());
                }
            }
            c if c.is_arbitrum() => {
                ...
                {
                    env.blocks.get_mut(&env.cfg.chain_id).unwrap().number = l1_block_number.to();
                }
            }
            _ => {}
        }
...
pub fn configure_tx_req_env(
    ...
    // XXX FIXME YSG
    env.tx.kind = ChainAddress(env.cfg.chain_id, to);

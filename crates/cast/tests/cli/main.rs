...
forgetest_async!(decode_external_libraries_with_cached_selectors, |prj, cmd| {
    let (api, handle) = match anvil::try_spawn(NodeConfig::test()).await {
        Ok(res) => res,
        Err(err) => {
            eprintln!(
                "skipping decode_external_libraries_with_cached_selectors: failed to spawn anvil ({err})"
            );
            return;
        }
    };
...
casttest!(odyssey_can_run_p256_precompile, |_prj, cmd| {
    ...
    if !output.get_output().status.success() {
        ...
        if stderr.contains("Attempted to create a NULL object.") {
            eprintln!(
                "skipping odyssey_can_run_p256_precompile: system proxy unavailable ({stderr})"
            );
            return;
        }

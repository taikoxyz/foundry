...
use revm::{context::JournalTr, interpreter::InstructionResult, primitives::ChainAddress, state::Bytecode};
...
impl Cheatcode for mockCall_0Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, '_>) -> Result {
        ...
        let acc = ccx.ecx.journaled_state.load_account(ChainAddress(chain_id, *callee))?;
...
impl Cheatcode for mockCall_1Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, '_>) -> Result {
        ...
        let chain_id = ccx.state.chain_id;
        ccx.ecx.journaled_state.load_account(ChainAddress(chain_id, *callee))?;
